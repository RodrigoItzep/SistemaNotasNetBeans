...
public class EstudianteService {
    private static final String BASE_URL = "http://localhost:8081/api/estudiantes";
    ...
    public List<Estudiante> getEstudiantes() throws Exception {
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            ...
            return mapper.readValue(is, new TypeReference<List<Estudiante>>() {});
        }
    }
    ...
    public Estudiante createEstudiante(Estudiante estudiante) throws Exception {
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            ...
            ClassicHttpResponse response = (ClassicHttpResponse) client.execute(request);
            InputStream is = response.getEntity().getContent();
            return mapper.readValue(is, Estudiante.class);
        }
    }

    // PUT actualizar estudiante
    public Estudiante updateEstudiante(int id, Estudiante estudiante) throws Exception {
        try (CloseableHttpClient client = HttpClients.createDefault()) {
    ...
    public String deleteEstudiante(int id) throws Exception {
        try (CloseableHttpClient client = HttpClients.createDefault()) {
